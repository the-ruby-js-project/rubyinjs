...
arr.last(2)                          #=> ['e', 'f']
 */
Array.prototype.last = function (count = 1) {
    result = this.slice(this.length-count, this.length);
    return (result.length == 1 ? result[0] : result);
};
...
Array.prototype.pop = function (count = 1) {
    return this.splice(this.length - count, count);
};
 ...
 */
Array.prototype.keep_if = function (func) {
    for (var i = 0; i < this.length; i++) {
        if (!func(this[i])) {
            this.splice(i, 1);
            i--;
...
c b a
 */
Array.prototype.reverse_each = function (func) {
...
Array.prototype.sample = function (count = 1) {
    ...
    for (var i = 0; i < count; i++) {
        // random numbers between 0 - (1 less then length)
        var rand_number = Math.floor(Math.random() * this.length);
        if (random_numbers.includes(rand_number)) {
            // redo the iteration
            i--;
        } else {
            result.push(this[rand_number]);
 ...
 */
Array.prototype.transpose = function (func = null) {
    var a= this;
    return(a[0].map((col, c) => a.map((row, r) => a[r][c])));
};
...
c
 */
Array.prototype.each = function (func = null) {
    this.forEach(func);
    return this;
};
